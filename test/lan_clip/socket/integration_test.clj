...
(deftest file-list-roundtrip-through-client-server-pipelines
  (testing "client encoder 输出应能被 server decoder 正确解码为 file-list Message"
    (let [node-id (UUID/randomUUID)
          secret "integration-test"
          temp-file (doto (File/createTempFile "test" ".txt")
                      (.deleteOnExit))
          _ (spit temp-file "integration file list")
          files (Collections/singletonList temp-file)
          ;; Client pipeline: encoder
          client-ch (EmbeddedChannel. (into-array ChannelHandler [(codec/->protocol-encoder node-id node-id secret)]))
          _ (.writeOutbound client-ch (into-array Object [files]))
          ^ByteBuf frame (.readOutbound client-ch)
      ...
      (try
        ...
        (is (> (count (:payload msg)) 0) "payload 应为非空 zip 字节")
        ;; 验证 ZIP magic
        (is (Arrays/equals (byte-array [0x50 0x4B 0x03 0x04]) (Arrays/copyOfRange ^bytes (:payload msg) 0 4)))
        (is (= node-id (:origin-node-id msg)))
        (is (= node-id (:sender-node-id msg)))
        (finally
          (.finish client-ch)
